...
android {
    ...
    testOptions {
        ...
        unitTests {
            ...
            // executes the real eviction/GC SQL. See app/src/test/java/app/swarsetu/data/ and
        ...
        managedDevices {
            localDevices {
                ...
                // Headless emulator for the accessibility suite (app.swarsetu.a11y): the Compose ATF
                ...
                // -PseedDemo=true -Pandroid.testInstrumentationRunnerArguments.package=app.swarsetu.a11y`.
...
room {
    ...
    // the flat schemas/app.swarsetu.data.MeshDatabase/<version>.json — same layout the ksp arg produced —
    // which the debug sourceSet below serves as a unit-test asset so MigrationTestHelper can read it.
    // Requires exportSchema = true on MeshDatabase; regenerate the checked-in schema by clearing app/schemas/
...
dependencies {
    ...
    // Offline STT engine (sherpa-onnx — next-gen Kaldi, Apache-2.0). Ships native .so for all ABIs;
    // Int8-quantized IndicConformer models in assets/ run fully offline. Used by stt/SherpaEngine.
    ...
    // by Room). See app/src/test/java/app/swarsetu/data/.
    ...
    // a11y package (app.swarsetu.a11y, @RequiresApi(34)). See .agents/context/testing.md.
    ...
    // UIAutomator black-box suite (app.swarsetu.uiauto): drives the real app process via resource-ids
