...
import app.swarsetu.ui.ISSUES_URL
import app.swarsetu.ui.PLAY_LISTING_URL
import app.swarsetu.ui.REPO_URL
 ...
 * (see the trigger in `SwarSetuApp`).
 *
 * When the gates pass it records the attempt and signals [ReviewPromptInbox]; `SwarSetuApp` observes that and
 * shows [app.swarsetu.ui.review.RateReviewDialog] over the chat list. The dialog's buttons open
...
class ReviewPrompter(
     ...
     * `SwarSetuApp` shows the dialog. Fires on each chat-list landing; the cheap pre-gates keep it a no-op in
