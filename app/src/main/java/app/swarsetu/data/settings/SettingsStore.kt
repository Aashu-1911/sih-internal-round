 ...
 * no longer persisted here — it is derived from the E2E keypair; see [app.swarsetu.identity.Identity].)
 */
class SettingsStore(
    ...
    /**
     * Device tags (see [app.swarsetu.identity.DeviceTag]) the user has blocked. Because a nodeId is
     ...
     * the peer's node id, or a group keyed by its "g-…" id (see [app.swarsetu.data.message.Conversations]).
     ...
     * notification, and back to true whenever the service (re)starts, so [app.swarsetu.mesh.BootReceiver]
     ...
     * review-prompt engagement window (see [app.swarsetu.review.ReviewPromptPolicy]). Deliberately a
