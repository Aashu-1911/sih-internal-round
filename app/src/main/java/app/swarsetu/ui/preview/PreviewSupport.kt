package app.swarsetu.ui.preview
...
/**
 * Shared wrapper for `@Preview` composables. Wraps [content] in [SwarSetuTheme] (so previews pick up the
 ...
 *
 * `SwarSetuTheme` uses `dynamicColor = false`, so a preview needs no `Context` — the static
 ...
 */
@Composable
fun SwarSetuPreview(content: @Composable () -> Unit) {
    SwarSetuTheme {
        Surface {
            content()
        }
