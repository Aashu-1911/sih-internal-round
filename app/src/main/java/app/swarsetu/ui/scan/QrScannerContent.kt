...
import app.swarsetu.ui.camera.CameraGate
import app.swarsetu.ui.camera.CameraMessage
 ...
 * [app.swarsetu.ui.verify.VerifyContactScreen] and [app.swarsetu.ui.profile.ProfileDetailsScreen]).
 * It is a plain composable rather than an Activity or a `Dialog`: screens in this app take lambdas and
 * `SwarSetuApp` owns navigation, and a camera `SurfaceView` inside a `Dialog` window has z-ordering quirks on
 ...
 * [CameraGate], shared with [app.swarsetu.ui.camera.PhotoCapture]. Decoding is [QrDecoder], which
