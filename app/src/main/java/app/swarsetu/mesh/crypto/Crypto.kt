...
/**
 * AES-256-GCM helper mirroring [app.swarsetu.data.crypto.DatabaseKey]'s parameters (12-byte random
 ...
 * Config kept explicit and in lockstep with [app.swarsetu.mesh.protocol.WireCodec] so the frozen wire
 * contract is spelled out, not implied by library defaults: `encodeDefaults = false` (the null-field
 * omission [app.swarsetu.mesh.crypto.MessageContent] relies on) and definite-length CBOR (iOS-codec
