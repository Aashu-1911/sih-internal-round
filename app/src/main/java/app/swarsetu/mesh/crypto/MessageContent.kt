 ...
 * CBOR-serialized and AES-256-GCM-encrypted into [app.swarsetu.mesh.protocol.EncEnvelope.ct]; the
 * cleartext [app.swarsetu.mesh.protocol.RelayEnvelope] only keeps the routing metadata (id, sender,
 * recipientId/group) that relays need.
 *
 * [v] versions the *decrypted plaintext schema* — deliberately independent of
 * [app.swarsetu.mesh.protocol.EncEnvelope.v] (the crypto scheme), so the two layers move
