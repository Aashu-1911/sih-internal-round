package app.swarsetu.mesh.crypto
 ...
 * key persistence lives in [app.swarsetu.data.crypto.IdentityKeyStore].
 *
 ...
 * envelope is *not* signed here — it travels inside an encrypted [app.swarsetu.mesh.protocol.ChatContent]
 * payload, and the single frame signature over the whole [app.swarsetu.mesh.protocol.RelayEnvelope]
...
class MessageCrypto(
     ...
     * canonical [app.swarsetu.mesh.protocol.WireEnvelope.signed] blob (the one signature that now
