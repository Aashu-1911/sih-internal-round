...
package app.swarsetu.mesh.protocol
...
object FrameType {
     ...
     * (see `app.swarsetu.mesh.PendingInbound`): the locally-delivered types only. PROFILE and KEY_REQ
...
/**
 * Whether this frame is carried for store-and-forward delivery (see `app.swarsetu.mesh.ForwardSync`).
 ...
 * real value rides sealed in [app.swarsetu.mesh.crypto.MessageContent.attachmentMime] and is what
...
data class ChatContent(
    ...
    // STT transcript of a voice note. Set by the sender after recording + transcription.
    // For encrypted DM/group, the real value rides inside [enc] ([MessageContent.voiceTranscript]).
    val voiceTranscript: String? = null,
)

/**
 * Content of a [FrameType.PROFILE] frame: the peer's display [name]/[status], optional [avatarHash]
 * (content hash of the avatar blob), [pubKey] (base64 [app.swarsetu.mesh.crypto.PublicKeyBundle];
 ...
 * and [app.swarsetu.mesh.crypto.MessageContent] (encrypted DM/group). A plain nested value.
 */
@Serializable
data class ReplyRef(
    val messageId: String,
 ...
 * (see [app.swarsetu.data.message.Conversations.groupIdFor]); [name] is set ONLY by an explicit
 ...
 * ctl DMs, [app.swarsetu.mesh.crypto.MessageContent] `CTL_GROUP_KEY`). Integrity needs no
 ...
 * [app.swarsetu.mesh.crypto.MessageContent], additive): a distribution (`CTL_GROUP_KEY`) carries
 ...
 * [app.swarsetu.mesh.crypto.MessageContent], additive): the sealed replacement for the cleartext
...
/**
 * A sealed profile update, carried as [app.swarsetu.mesh.crypto.MessageContent.pr] under
 ...
 * content key encrypts the [app.swarsetu.mesh.crypto.MessageContent] with AES-256-GCM into [ct]
 ...
 * to interoperate (the [app.swarsetu.mesh.wifiaware.WifiAwareTransport] service name is bumped in lockstep).
 */
object WireCodec {
    @PublishedApi
