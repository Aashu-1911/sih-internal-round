 ...
 * ([app.swarsetu.mesh.link.LinkSocket]) share it unchanged.
 *
 * Each record is `[type:1][len:4 big-endian][payload:len]`. [Type.FRAME] carries one CBOR
 * [app.swarsetu.mesh.protocol.WireEnvelope]; a file is streamed as [Type.FILE_HEADER] (a
 ...
 *
 * Pure (no Android), so the codec is JVM-unit-testable ([app.swarsetu.LinkFramingTest]).
 */
internal object LinkFraming {
    ...
    enum class Type(
         ...
         * ([app.swarsetu.mesh.protocol.Protocol.advertise] bytes), so an accept-any responder — which
     ...
     * between-chunk interleave/pace points (see [app.swarsetu.mesh.link.FramedLink]) are frequent — the
