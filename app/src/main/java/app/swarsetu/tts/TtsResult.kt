...
sealed class TtsLanguageCapability {
    ...
    data class Supported(val engineName: String, val voiceName: String?) : TtsLanguageCapability()

    /** The language is supported by the engine, but the voice data is missing and must be downloaded. */
    data class MissingData(val engineName: String) : TtsLanguageCapability()

    /** The language is not supported by the engine. */
    data class Unsupported(val reason: String) : TtsLanguageCapability()

    /** An error occurred checking the language state. */
    data class Error(val errorMessage: String) : TtsLanguageCapability()
}
...
sealed class TtsResult {
    ...
    data class Interrupted(val reason: String = "Interrupted by another request or alert") : TtsResult()
