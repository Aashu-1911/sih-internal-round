...
/**
 * Base for the **UIAutomator** black-box suite (`app.swarsetu.uiauto`). Where the Compose suite
 * ([app.swarsetu.ui.SeededUiTest]) drives the app *in-process* via the semantics tree, this drives the
 ...
 *
 * Selectors: `testTagsAsResourceId` is enabled at the NavHost root (`ui/SwarSetuApp.kt`), so every Compose
...
abstract class SeededUiAutomatorTest {
     ...
     * process *is* `app.swarsetu` — stopping it would kill the test mid-teardown and surface as
    ...
    protected fun bringToForeground() {
        ...
        // Recents. No demo_route, so SwarSetuApp doesn't re-navigate: the current screen + in-memory state are
