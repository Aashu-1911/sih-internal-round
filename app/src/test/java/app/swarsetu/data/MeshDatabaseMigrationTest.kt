 ...
 * v1 forward every schema bump ships a tested [MeshMigrations] entry validated here — one per bump, plus the
 * current-schema smoke test that exercises the schema-export pipeline end-to-end: `createDatabase(version)`
 * rebuilds the DB from the checked-in `app/schemas/app.swarsetu.data.MeshDatabase/<version>.json`, proving
 ...
 * change lands, add a [MeshMigrations] entry and fill in the template below — `runMigrationsAndValidate` then
...
class MeshDatabaseMigrationTest {
    ...
    val helper =
        MigrationTestHelper(
            ...
            databaseClass = MeshDatabase::class,
        )

    @Test
    fun `the current schema (v6) creates and opens from the exported JSON`() {
        val version = 6 // MeshDatabase @Database(version = 6) — bump alongside the DB (its retention is CLASS,
    ...
    fun `migrate 1 to 2 preserves existing rows and adds the ratchet and group-ratchet schemas`() {
        ...
        helper.runMigrationsAndValidate(2, listOf(MeshMigrations.MIGRATION_1_2)).use { c ->
    ...
    fun `migrate 2 to 3 preserves existing rows and adds the group-roots schema`() {
        ...
        helper.runMigrationsAndValidate(3, listOf(MeshMigrations.MIGRATION_2_3)).use { c ->
    ...
    fun `migrate 3 to 4 keeps messages and leaves their delivery plane unknown`() {
        ...
        helper.runMigrationsAndValidate(4, listOf(MeshMigrations.MIGRATION_3_4)).use { c ->
    ...
    fun `migrate 4 to 5 keeps messages and leaves their voice columns null`() {
        ...
        helper.runMigrationsAndValidate(5, listOf(MeshMigrations.MIGRATION_4_5)).use { c ->
    ...
    fun `migrate 5 to 6 adds an empty message_receipts table and keeps an already-ticked message`() {
        ...
        helper.runMigrationsAndValidate(6, listOf(MeshMigrations.MIGRATION_5_6)).use { c ->
