 ...
 *
 * [SwarSetuTheme] tracks the system dark theme through `isSystemInDarkTheme()`, but `res/values/themes.xml`
 ...
 * straight off disk (module-dir cwd, like [app.swarsetu.moderation.WordListTest]) — a fast,
...
class SplashThemeTest {
    ...
    fun launchThemePaintsBothWindowBackgroundsFromTheNightAwareColor() {
        ...
        listOf("android:windowBackground", "android:windowSplashScreenBackground").forEach { attr ->
            assertEquals(
                "Theme.SwarSetu must paint $attr from the night-aware colour",
                "@color/$SPLASH_COLOR",
                items[attr],
            )
        }
    ...
    /** `Theme.SwarSetu`'s own `<item>` overrides, keyed by attribute name. */
    private fun launchThemeItems(): Map<String, String> {
        val style =
            elements("values/themes.xml", "style").firstOrNull { it.getAttribute("name") == "Theme.SwarSetu" }
                ?: error("no <style name=\"Theme.SwarSetu\"> in res/values/themes.xml")
