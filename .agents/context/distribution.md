...
`metadata/app.swarsetu.yml` — our copy is documentation to F-Droid's bot, nothing more. An alpha tagged
...
`metadata/app.swarsetu.yml`. That invariant was silently broken once: the new-app review (MR 43609)
...
```bash
diff <(grep -v '^\s*#' .fdroid.yml | grep -v '^$') <(grep -v '^$' ../fdroiddata/metadata/app.swarsetu.yml)
```
