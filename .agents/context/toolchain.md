...
- **DI is Koin, not Hilt.** Hilt's Gradle plugin is broken on AGP 9.x in this window
  ...
  processor, so it can't be broken by AGP. Koin is started in `SwarSetuApplication`; modules live in
...
`MeshDatabase` and Room ≥ 2.7.0-alpha13 for KSP2 support (we're on 2.8.4 / KSP 2.3.11). With only build
types (no product flavors) it writes to the flat `app/schemas/<db-class>/<version>.json` — the same layout
the KSP arg produced — so the debug-asset wiring and `MeshDatabaseMigrationTest` are unchanged. **Gotcha:**
