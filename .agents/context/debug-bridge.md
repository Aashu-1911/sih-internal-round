...
`-p app.swarsetu`); the reply prints on stdout as `Broadcast completed: … data="{…}"` and is also
...
- `…debug.SENDIMG` — sends a real **image attachment** with no UI (a locked device can't drive the photo
  ...
  app.swarsetu sh -c 'cat > files/img.jpg'"`, then pass `--es path /data/data/app.swarsetu/files/img.jpg`.
...
adb -s A shell "am broadcast -a app.swarsetu.debug.SEND  -p app.swarsetu --es text 'hi there 😀' --es conv nearby"
adb -s B shell  am broadcast -a app.swarsetu.debug.STATE -p app.swarsetu --es conv nearby
...
for d in A B; do adb -s $d shell "am broadcast -a app.swarsetu.debug.SPOOL -p app.swarsetu \
  --es url ws://<lan-ip>:9470/spool/v1 --ez on true"; done
adb -s B shell am broadcast -a app.swarsetu.debug.SPOOL -p app.swarsetu
...
The root sets `testTagsAsResourceId` (in `SwarSetuApp`), so `Modifier.testTag`s surface in `uiautomator dump`
as `resource-id="<tag>"` (the bare tag — some Android/uiautomator versions prefix it
`app.swarsetu:id/<tag>`, so a matcher should accept either form). Tagged so far: `chat_input`, `chat_send`, `chat_row_<conversationId>` (e.g.
...
separate window whose semantics root is *not* the `SwarSetuApp` node that sets `testTagsAsResourceId`, so a
...
`adb shell am start -n app.swarsetu/.MainActivity --es demo_route chat/<id>` opens a thread directly
