...
- **The in-memory test DB skips SQLCipher.** `RoomDbTest` builds via `Room.inMemoryDatabaseBuilder(...)` with
  **no** `openHelperFactory`, so it uses Robolectric's framework SQLite — no passphrase, no `libsqlcipher.so`.
  The eviction/GC SQL runs identically (SQLCipher only encrypts at rest). Never call `MeshDatabase.build()` in
  a test. Call `suspend` DAO methods inside `runTest { }`.
- **`robolectric.properties` forces `application=android.app.Application`.** The real `SwarSetuApplication.onCreate`
  ...
  with compileSdk.
- **`exportSchema = true`** on `MeshDatabase` + the Room Gradle plugin's
  `room { schemaDirectory("$projectDir/schemas") }` emit
  `app/schemas/app.swarsetu.data.MeshDatabase/<version>.json` (checked in). Regenerate by clearing
...
- **`MigrationTestHelper` reads the schema from *debug*-variant assets** —
  ...
  **debug** assets (unit tests run against debug) but **not** the `test` source set's own assets; release APKs
  ...
  v1 forward every `@Database` bump MUST add a tested `Migration` to `MeshMigrations` (`data/MeshMigrations.kt`,
  `ALL` is empty at launch) and a from→to case to `MeshDatabaseMigrationTest` — a missing migration throws at
...
- **Isolated FTL target.** On Firebase Test Lab this package is run **on its own**
  (`--test-targets "package app.swarsetu.uiauto"`) and **excluded** from the default Compose run
  (`notPackage app.swarsetu.uiauto`) so black-box system-UI flakiness never reddens it. Both use the
...
- **Run locally**: `./gradlew :app:connectedDebugAndroidTest -PseedDemo=true
  -Pandroid.testInstrumentationRunnerArguments.package=app.swarsetu.uiauto` (drop the `-P…package` arg
...
- **The message-request notification seam.** The radio-less build never runs `InboundPipeline` (the only
  ...
  **`REQNOTIF`** (`DebugBridgeReceiver`) writes synthetic unaccepted inbound DMs and posts the real
  heads-up: `adb shell am broadcast -p app.swarsetu -a app.swarsetu.debug.REQNOTIF --ei count 1`.
...
- **Run locally** (headless emulator, no physical device):
  ...
  -Pandroid.testInstrumentationRunnerArguments.package=app.swarsetu.a11y`.
