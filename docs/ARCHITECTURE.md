...
- **Room** (`MeshDatabase`, `knit.db`) — **encrypted at rest with SQLCipher**: the passphrase is a
  ...
  tested `Migration` in `MeshMigrations` (validated by `MeshDatabaseMigrationTest`) — a missing one makes
...
- **Compose + Material 3, Navigation Compose.** `SwarSetuApp` hosts a `NavHost` with routes
...
- **Coil 3** is configured app-wide in `SwarSetuApplication` with the `AnimatedImageDecoder` so message
  GIFs/WebP animate.
...
- All cleaned up in `onDestroy` (including `MeshManager.stop()`, which cancels pending relays). The
  message notification channels are registered up front in `SwarSetuApplication` so they appear in system
