...
with a tested `MeshMigrations` entry.
...
"and a spool" claim — a spool never saw the field. The size half stands.) The DB bumps (v4 → v5) are local only, with a tested `MeshMigrations`
entry.
